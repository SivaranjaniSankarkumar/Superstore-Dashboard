...
if not region and not state and not city:
    ...
    filtered_df=df3[df["State"].isin(state) & df3["City"].isin(state)]
elif city:
...
with st.expander("Viewdata of time series analysis"):   
    st.write(linechart.T.style.background_gradient(cmap="Blues"))
    csv=linechart.to_csv(index=False).encode('utf-8')
    st.download_button("Download data",data=csv,file_name="linechart.csv",mime="text/csv",help="Click here to download the data as a CSV file")   
...
with st.expander("Summary_table:"):
    df_sample=df[0:5][["Region","Sales","City","Category","State","Profit","Quantity"]]
...
data1["layout"].update(title="Relationship between Sales and Proft using Scatter Plot",titlefont=dict(size=20),
                           xaxis=dict(title="Sales",titlefont=dict(size=20)),yaxis=dict(title="Sales",titlefont=dict(size=20)))
